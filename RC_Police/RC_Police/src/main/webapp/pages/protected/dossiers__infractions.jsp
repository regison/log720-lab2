...
<template:insert template='/shared/templates/baseTemplate.jsp'>
	...
	<template:put name='content' >
		...
		<jsp:useBean id="infractions" class="ca.etsmtl.log720.RC_Police.components.InfractionList" scope="request" />
		...
		<div class="col-xs-12">
		        ...
		        <form method="post" action="dossier__infraction" class="form">
								 	...
								 	<div class="table-responsive">
										<table class="table-striped table-bordered sortable col-xs-12" id="dataGrid-dossiers">
											...
											<tbody>
												...
												<c:forEach var="row" items="${infractions.getInfraction()}" >
													...
													<tr id="${row.id}">
														<td>
															<input class="checkbox checkbox-selection" 
																	...
																	id="${row.id}" 
																	name="${row.id}"  
																	<% if( dossier.getInfractions().contains( infractions.getInfraction().get(rowNumber) )){%> 
																		checked
																	<%} %>
																	/>
														</td>
